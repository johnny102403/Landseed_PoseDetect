				...
				if (keypoints.getSize(0)>0) {
					...
					if (personRectangle.area() > 0)
					{
						...
						*/
						...
						// Draw lines
						for (auto pair = 7; pair < 13; pair += 2)
						{
						...
						for (auto part = 0; part < numberKeypoints; part++)
						{
							...
							if (keypoints[faceIndex + 2] > threshold)
							{
								...
								if (part == 14 || part == 1 || part == 8 || part == 9 || part == 10) {
						...
						*/
						...
						//LHip
						if (posePoint_X[1] > 0 && posePoint_Y[1] > 0 && posePoint_X[11] > 0 && posePoint_Y[11] > 0 && posePoint_X[15] > 0 && posePoint_Y[15] > 0) {
							double num = countAngle(posePoint_X[15], posePoint_Y[15], posePoint_X[1], posePoint_Y[1], posePoint_X[11], posePoint_Y[11]);
							snprintf(output, 50, "%.2f", num);
							//if (num < 170) {
							center.x = posePoint_X[11] + 55;
							center.y = posePoint_Y[11] + 55;
							cv::putText(frameR, output, center, 0, 0.5, color, 2);
							cv::putText(frameG, output, center, 0, 0.5, color, 2);
							cv::putText(frameB, output, center, 0, 0.5, color, 2);
							//}
